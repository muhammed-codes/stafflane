...
from stafflane.methods import dynamic_attr, get_stafflane_model_class
from stafflane_audit.models import AccountBlockUnblock, HistoryTrackingFields
from stafflane_auth.models import StafflaneUser
from stafflane_documents.forms import (
    DocumentForm,
    DocumentRejectForm,
    DocumentRequestForm,
    DocumentUpdateForm,
...
def allowances_deductions_tab(request, pk):
    ...
    if basic_pay:
        ...
        Allowance = get_stafflane_model_class(app_label="payroll", model="allowance")
        ...
        Deduction = get_stafflane_model_class(app_label="payroll", model="deduction")
...
@login_required
@manager_can_enter("stafflane_documents.view_documentrequest")
def document_request_view(request):
    ...
    documents = filtersubordinates(
        request=request,
        perm="stafflane_documents.view_documentrequest",
        queryset=documents,
    )
...
@login_required
@hx_request_required
@manager_can_enter("stafflane_documents.add_documentrequest")
def document_request_create(request):
    ...
    form = choosesubordinates(request, form, "stafflane_documents.add_documentrequest")
    if request.method == "POST":
        form = DocumentRequestForm(request.POST)
        form = choosesubordinates(
            request, form, "stafflane_documents.add_documentrequest"
        )
        if form.is_valid():
            form = form.save()
...
@login_required
@hx_request_required
@manager_can_enter("stafflane_documents.change_documentrequest")
...
@login_required
@hx_request_required
@owner_can_enter("stafflane_documents.view_document", Employee)
...
@login_required
@hx_request_required
@owner_can_enter("stafflane_documents.add_document", Employee)
...
def document_delete(request, id):
    ...
    try:
        ...
        if not request.user.has_perm("stafflane_documents.delete_document"):
...
def file_upload(request, id):
    ...
    if not can_access_document(
        request, document_item, "stafflane_documents.change_document"
    ):
        return StafflaneRedirect(
...
def view_file(request, id):
    ...
    if not can_access_document(
        request, document_obj, "stafflane_documents.view_document"
    ):
        return StafflaneRedirect(
...
@login_required
@hx_request_required
@manager_can_enter("stafflane_documents.add_document")
...
def employee_user_group_assign_delete(_, obj_id):
    ...
    """
    user = StafflaneUser.objects.get(id=obj_id)
    user.groups.clear()
...
def employee_account_block_unblock(request, emp_id):
    ...
    if not employee:
        ...
        return redirect(f"{reverse('employee-view')}?view=list")
    user = get_object_or_404(StafflaneUser, id=employee.employee_user_id.id)
    if not user:
        messages.info(request, _("Employee not found"))
        return redirect(f"{reverse('employee-view')}?view=list")
...
def employee_view_update(request, obj_id, **kwargs):
    ...
    if (
        ...
        return response
    return StafflaneRedirect(request, fallback_url="/employee/employee-view")
...
def employee_delete(request, obj_id):
    ...
    return StafflaneRedirect(request, fallback_url=f"/view={view}")
        ...
        for models in related_models:
            ...
            if field_name:
                ...
                if (
                    ...
                    Recruitment = get_stafflane_model_class(
                    ...
                    Stage = get_stafflane_model_class(
                    ...
                    OnboardingStage = get_stafflane_model_class(
                    ...
                    OnboardingTask = get_stafflane_model_class(
    ...
    if request.method == "POST":
        ...
        for employee_dict in employee_dicts:
            try:
                ...
                existing_user = StafflaneUser.objects.filter(username=email).first()
    ...
    for employee in employees:
        for column_value, column_name in selected_columns:
            ...
            if isinstance(value, date):
                try:
                    data = value.strftime(
                        settings.STAFFLANE_DATE_FORMATS.get(date_format, "%Y-%m-%d")
                    )
                except Exception:
                    data = str(value)
...
def leave_today_count(request):
    ...
    if apps.is_installed("leave"):
        LeaveRequest = get_stafflane_model_class(app_label="leave", model="leaverequest")
...
def bonus_points_tab(request, pk):
    ...
    try:
        ...
        if apps.is_installed("payroll"):
            Reimbursement = get_stafflane_model_class(
