...
from stafflane.decorators import owner_can_enter
from stafflane_api.api_decorators.base.decorators import permission_required
from stafflane_api.api_methods.employee.methods import get_next_badge_id
...
class DocumentRequestAPIView(APIView):
    ...
    @manager_permission_required("stafflane_documents.add_documentrequests")
    def post(self, request):
    ...
    @manager_permission_required("stafflane_documents.change_documentrequests")
    def put(self, request, pk):
        document_request = self.get_object(pk)
...
class DocumentAPIView(APIView):
    ...
    @manager_or_owner_permission_required(
        DocumentRequest, "stafflane_documents.add_document"
    )
    def post(self, request):
    ...
    @method_decorator(owner_can_enter("stafflane_documents.change_document", Employee))
    def put(self, request, pk):
        document = self.get_object(pk)
    ...
    @method_decorator(owner_can_enter("stafflane_documents.delete_document", Employee))
    def delete(self, request, pk):
        document = self.get_object(pk)
        document.delete()
        return Response(status=status.HTTP_204_NO_CONTENT)
...
class DocumentRequestApproveRejectView(APIView):
    ...
    @manager_permission_required("stafflane_documents.add_document")
