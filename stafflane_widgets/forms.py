...
forms.Select.option_template_name = "stafflane_widgets/stafflane_select_option.html"


class StafflaneForm(forms.Form):
...
class StafflaneModelForm(forms.ModelForm):
    def clean(self) -> Dict[str, Any]:
        for field_name, field_instance in self.fields.items():
            if isinstance(field_instance, StafflaneMultiSelectField):
                self.errors.pop(field_name, None)
