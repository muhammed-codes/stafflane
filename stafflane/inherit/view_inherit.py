"""
stafflane/inherit/view_inherit.py

StafflaneViewInheritMixin — add to an extension class to enable view replacement.

Two _inherit modes
------------------

Mode 1  _inherit = True   (direct subclass replacement)
  Include StafflaneViewInheritMixin as the first base alongside the target view.
  ...
  Full Python inheritance — super(), MRO, and attribute lookup all work normally.
    ...
    from stafflane.inherit import StafflaneViewInheritMixin

    class AssetListExtension(StafflaneViewInheritMixin, AssetListView):
        _inherit = True
...
Mode 2  _inherit = "app_label.ClassName"  (indirect attribute injection)
  ...
  The target view must be registered via register_view() to be reachable.
    ...
    class AssetListExtension(StafflaneViewInheritMixin):
        _inherit = "asset.AssetList"
        columns = ["name", "asset_status", "warranty_expiry_date"]
"""

from stafflane.inherit.view_registry import register_extension, register_replacement
...
class StafflaneViewInheritMixin:
    """
    Mixin that enables _inherit on Stafflane CBV extension classes.
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        if inherit is True:
            parent = next(
                (b for b in cls.__bases__ if b is not StafflaneViewInheritMixin),
                None,
            )
