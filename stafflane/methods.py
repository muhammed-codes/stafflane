...
def stafflane_users_with_perms(permissions):
    """
    ...
    users_with_permissions = StafflaneUser.objects.filter(is_superuser=True)

    # Filter users based on the permissions list
    for perm in permissions:
        app_label, codename = perm.split(".")
        users_with_permissions |= StafflaneUser.objects.filter(
