...
class AssignPermission(Form):
    ...
    employee = StafflaneMultiSelectField(
    ...
    def save(self):
        ...
        users = StafflaneUser.objects.filter(id__in=user_ids)
        for user in users:
...
class JobPositionMultiForm(ModelForm):
    ...
    department_id = StafflaneMultiSelectField(
        queryset=Department.objects.all(),
...
class ChangeUsernameForm(forms.Form):
    ...
    def clean_password(self):
        ...
        if StafflaneUser.objects.filter(username=username).exists():
...
class AnnouncementForm(ModelForm):
    ...
    employees = StafflaneMultiSelectField(
        queryset=Employee.objects.all(),
        widget=StafflaneMultiSelectWidget(
            filter_route_name="employee-widget-filter",
            filter_class=EmployeeFilter,
    ...
    def clean(self):
        ...
        if isinstance(self.fields["employees"], StafflaneMultiSelectField):
...
class PassWordResetForm(forms.Form):
    ...
    def save(
        ...
        user = StafflaneUser.objects.get(username=username)
...
class PenaltyAccountForm(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if apps.is_installed("leave") and employee:
            LeaveType = get_stafflane_model_class(app_label="leave", model="leavetype")
