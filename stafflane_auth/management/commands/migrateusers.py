...
from stafflane_auth.models import StafflaneUser, LegacyUser


class Command(BaseCommand):
    help = "Migrate users from LegacyUser (auth_user) to StafflaneUser, including groups and permissions."
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            for old_user in LegacyUser.objects.all():
                if StafflaneUser.objects.filter(username=old_user.username).exists():
                    skipped_count += 1
                    continue
                ...
                new_user = StafflaneUser.objects.create(
                    id=old_user.id,
