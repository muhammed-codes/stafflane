# `stafflane_dbtemplate` — Database-backed Django templates
...
| **`stafflane.contrib.utils.middlewares`** | Loader reads `request` from `_thread_local`; ensure that middleware runs before template rendering where DB resolution is needed. |
...
- **`INSTALLED_APPS`**: include `django.contrib.sites` and `stafflane_dbtemplate` (or the app’s dotted path your project uses).
- **`STAFFLANE_DBTEMPLATE_CACHE_BACKEND`**: optional; name of a key in `CACHES` (default: `"default"`). See `conf.py`.
- **`STAFFLANE_DBTEMPLATE_AUTO_POPULATE_CONTENT`**: optional; if true, empty content can be filled from disk on save. See `conf.py`.

### Registering the loader

Add the loader class to the Django template engine’s `OPTIONS["loaders"]`. The import path is typically:

`stafflane_dbtemplate.loaders.Loader`

Place it according to product rules: usually **first among custom loaders** so active DB templates override packaged files. Exact `TEMPLATES` structure depends on your Stafflane project; search the repo for `Loader` or `stafflane_dbtemplate.loaders`.
...
4. **Cache** (`utils/cache.py`): keys are `stafflane_dbtemplate::<slugified_name>::<site_pk|global>::<slugified_lang>`. Invalidation must cover **all locale variants** in `LANGUAGES` plus `LANGUAGE_CODE`, because the loader uses `get_language()` while many rows use blank `language`.
...
Paths are relative to **`stafflane_dbtemplate/`**.

| Path | Role |
|------|------|
| **`__init__.py`** | Package docstring summarising resolution order and localhost / non-matching-site behaviour. |
| **`apps.py`** | `StafflaneDBTemplateConfig` (`AppLauncher`): app label, verbose name, **`auto_import_modules = ["signals"]`** so receivers load at startup. |
...
| **`conf.py`** | `STAFFLANE_DBTEMPLATE_*` settings → cache backend name, `AUTO_POPULATE_CONTENT`, **`get_cache()`**. |
...
| **`templates/admin/stafflane_dbtemplate/template/*.html`** | Admin UI: **`restore_version.html`**, **`diff.html`**, **`preview.html`**. |
...
- **Key shape**: `stafflane_dbtemplate::{slugify(template_name)}::{site_id|global}::{slugify(language_or_all)}`.
