...
from stafflane_meet.filters import GoogleMeetingFilter
from stafflane_meet.form import GoogleCloudCredentialForm, GoogleMeetingForm
from stafflane_meet.models import GoogleCloudCredential, GoogleCredential, GoogleMeeting
from stafflane_views.generic.cbv import views
...
class GMeetCredentialSectionView(views.StafflaneSectionView):
    """
    View for displaying the Google Meet credentials section.

    Inherits from `StafflaneSectionView` to manage the section for Google Cloud credentials in the user interface.
...
@method_decorator(login_required, name="dispatch")
@method_decorator(check_integration_enabled(app_name="stafflane_meet"), name="dispatch")
@method_decorator(
    permission_required("stafflane_meet.view_googlecloudcredential"), name="dispatch"
)
class GmeetCredentialNavView(views.StafflaneNavView):
    """
    View for the Google Meet credentials nav view.

    Inherits from `StafflaneNavView` to manage nav view for Google Cloud credentials. Handles dynamic attributes
...
class GmeetCredentialListView(views.StafflaneListView):
    """
    View for listing Google Meet credentials.

    Inherits from `StafflaneListView` to manage the listing of Google Cloud credentials with actions like edit and delete.
...
@method_decorator(check_integration_enabled(app_name="stafflane_meet"), name="dispatch")
@method_decorator(
    permission_required("stafflane_meet.add_googlecloudcredential"), name="dispatch"
)
class GoogleCredentialsFormView(views.StafflaneFormView):
    """
    View for creating or updating Google Cloud credentials.

    Inherits from `StafflaneFormView` to manage the form for creating or updating Google Cloud credentials.
    ...
    template_name = "generic/stafflane_form.html"
...
@method_decorator(login_required, name="dispatch")
@method_decorator(
    permission_required("stafflane_meet.add_googlemeeting"), name="dispatch"
)
@method_decorator(check_integration_enabled(app_name="stafflane_meet"), name="dispatch")
class GmeetFormView(views.StafflaneFormView):
    """
    View for creating or updating Google Meet meetings.

    Inherits from `StafflaneFormView` to manage the form for creating or updating Google Meet meetings.
    Handles form validation, success, and failure responses. Displays success or error messages accordingly.
...
@method_decorator(check_integration_enabled(app_name="stafflane_meet"), name="dispatch")
@method_decorator(
    permission_required("stafflane_meet.view_googlemeeting"), name="dispatch"
)
class GmeetSectionView(views.StafflaneSectionView):
    """
    View for displaying the Google Meet section.

    Inherits from `StafflaneSectionView` to manage the section for Google Meet in the user interface.
...
class GmeetNavView(views.StafflaneNavView):
    """
    Nav view for Google Meet.

    Inherits from `StafflaneNavView` to manage nav bar for Google Meet view. Handles dynamic attributes for creating
...
class GmeetListView(views.StafflaneListView):
    """
    View for listing Google Meetings.

    Inherits from `StafflaneListView` to manage the listing of Google meetings with actions like edit, delete, and view meeting link.
...
class GmeetDetailedView(views.StafflaneDetailedView):
    """
    Detailed view for Google Meetings.

    Inherits from `StafflaneDetailedView` to manage the detailed view of a specific Google Meeting.
