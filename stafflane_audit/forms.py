...
from stafflane_audit.models import AuditModelConfig, AuditTag, StafflaneAuditInfo
...
class HistoryForm(forms.Form):
    ...
    def as_history_modal(self, *args, **kwargs):
        ...
        """
        context = {"form": self}
        table_html = render_to_string("stafflane_audit/stafflane_audit_log.html", context)
        return table_html
...
def _eligible_models():
    """Return concrete StafflaneModel subclasses that can be audit-tracked."""
    ...
    for model in django_apps.get_models():
        if not issubclass(model, StafflaneModel) or model._meta.abstract:
            continue
...
def model_choices():
    from stafflane_audit.registry import DEFAULT_TRACKED_MODELS
...
def field_choices_for(app_label, model_name):
    ...
    try:
        ...
        return []
    skip = {"stafflane_history", "objects"}
    return [
