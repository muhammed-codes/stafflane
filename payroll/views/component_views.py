...
from stafflane.methods import dynamic_attr, get_stafflane_model_class, get_urlencode
...
def update_allowance(request, allowance_id, **kwargs):
    ...
    is_htmx = request.headers.get("HX-Request") is not None
    if not instance:
        return StafflaneRedirect(request, message=_("Allowance not found."))
...
#     ):
#         return return StafflaneRedirect(request)
...
def update_deduction(request, deduction_id, **kwargs):
    ...
    is_htmx = request.headers.get("HX-Request") is not None
    if not instance:
        return StafflaneRedirect(request, message=_("Deduction not found."))
...
def validate_start_date(request):
    ...
    try:
        ...
        return StafflaneRedirect(request, message=_("Invalid Request"))
...
def add_bonus(request):
    ...
    if payslip_id != "None" and payslip_id:
        ...
        if not instance:
            return StafflaneRedirect(request, _("Payslip not found"))
...
def view_installments(request):
    ...
    if not loan:
        return StafflaneRedirect(request, message=_("Loan not found."))
...
def get_assigned_leaves(request):
    ...
    if apps.is_installed("leave"):
        AvailableLeave = get_stafflane_model_class(
...
def reimbursement_individual_view(request, instance_id):
    ...
    if not reimbursement:
        return StafflaneRedirect(request, message=_("Reimbursement request not found."))
    requests_ids_json = request.GET.get("instances_ids")
    if requests_ids_json:
        requests_ids = json.loads(requests_ids_json)
