"""
stafflane_automations/views/cbvs.py
"""
...
from stafflane_automations import models
from stafflane_automations.filters import AutomationFilter
from stafflane_automations.forms import AutomationForm
from stafflane_views.generic.cbv import views


@method_decorator(login_required, name="dispatch")
@method_decorator(
    permission_required("stafflane_automations.view_mailautomation"), name="dispatch"
)
class AutomationSectionView(views.StafflaneSectionView):
    """
    ...
    template_name = "stafflane_automations/section_view.html"
...
class AutomationNavView(views.StafflaneNavView):
    """
    AutomationNavView
    """
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.actions = []
        if self.request.user.has_perm("stafflane_automations.add_mailautomation"):
            self.create_attrs = f"""
...
@method_decorator(login_required, name="dispatch")
@method_decorator(
    permission_required("stafflane_automations.change_mailautomation"), name="dispatch"
)
class AutomationFormView(views.StafflaneFormView):
    """
    AutomationFormView
    """
    ...
    template_name = "stafflane_automations/automation_form.html"
...
class AutomationListView(views.StafflaneListView):
    """
    AutomationListView
    """
...
class AutomationDetailedView(views.StafflaneDetailedView):
    """
...
@method_decorator(
    permission_required("stafflane_automations.add_mailautomation"), name="dispatch"
)
class LoadAutomationsView(View):
    template_name = "stafflane_automations/load_automation.html"
    ...
    def post(self, request):
        ...
        for template_json in templates_raw:
            if template_json["pk"] in required_template_pks:
                ...
                existing = StafflaneMailTemplate.objects.filter(
        ...
        for automation_json in selected_automations:
            ...
            mail_template = StafflaneMailTemplate.objects.filter(
                body=template_body
            ).first()
