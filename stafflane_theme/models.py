"""
Models for the stafflane_theme app
"""

from django.core.exceptions import ValidationError
from django.core.validators import RegexValidator

# Create your stafflane_theme models here.
...
class StafflaneColorTheme(StafflaneModel):
    """
    Model to store predefined color themes for Stafflane
    """
    ...
    class Meta:
        """
        Meta option for StafflaneColorTheme model
        """
    ...
    def save(self, *args, **kwargs):
        ...
        with transaction.atomic():
            if self.is_default:
                query = StafflaneColorTheme.objects.filter(is_default=True)
                if self.pk:
                    query = query.exclude(pk=self.pk)
                query.update(is_default=False)
            super().save(*args, **kwargs)
            if self.is_default and self.pk:
                StafflaneColorTheme.objects.filter(is_default=True).exclude(
                    pk=self.pk
...
class CompanyTheme(StafflaneModel):
    """
    ...
    def get_default_theme(cls):
        ...
        Returns the theme object or None
        """
        return StafflaneColorTheme.get_default_theme()
    ...
    def get_theme_for_company(cls, company):
        ...
        # return StafflaneColorTheme.objects.filter(
        #     name="Coral Red Theme (Default)"
        # ).first()
        return StafflaneColorTheme.objects.filter(is_default=True).first()
